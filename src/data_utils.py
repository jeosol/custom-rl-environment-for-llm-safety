...
def preprocess_anthropic_hh(sample):
    ...
    if assistant_marker in chosen_str:
        ...
        prompt = chosen_str.split(assistant_marker)[0] + assistant_marker
        chosen_response = chosen_str.split(assistant_marker)[-1]
        rejected_response = rejected_str.split(assistant_marker)[-1]
    else:
